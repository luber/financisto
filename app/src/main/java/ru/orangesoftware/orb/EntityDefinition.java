...
//	final JoinEntity[] joinEntities;
	...
	final StringBuilder sqlTablesBuilder;
	final HashMap<String, String> columnProjectionMap = new HashMap<>();
	...
	private EntityDefinition(Constructor<?> constructor, String tableName, FieldInfo idField, FieldInfo[] fields) {
		...
		this.fields = fields;
		this.sqlTablesBuilder = new StringBuilder();
...
//		this.joinEntities = prepareJoinEntities();
	...
	public String getTables(){
		return sqlTablesBuilder.toString();
	}
...
//	}
	...
	protected String prepareSqlQuery() {
		...
		sbColumns.append("e").append(".").append(idField.columnName).append(" as ").append(DEFAULT_ID_COLUMN_NAME);

		String qualifiedCol = getQualifiedColName("e", idField.columnName);
		columnProjectionMap.put(qualifiedCol, qualifiedCol + " as " + DEFAULT_ID_COLUMN_NAME);

		sqlTablesBuilder.append(tableName).append(" as e");
		prepareSqlQuery(this, sbColumns, sqlTablesBuilder, tableName, "e", true);

		return queryBuilder.append(sbColumns).append(" from ").append(sqlTablesBuilder).toString();
	}

	private String getQualifiedColName(String tableName, String columnName) {
		return tableName + "." + columnName;
	}

	protected void prepareSqlQuery(EntityDefinition ed, StringBuilder sbColumns, StringBuilder sbJoins, String tableName, String pe, boolean required) {
		FieldInfo[] fields = ed.fields;
		for (FieldInfo f : fields) {
			if (f.type.isPrimitive()) {
				appendColumn(sbColumns, pe, f.columnName);

				String qualifiedCol = getQualifiedColName(tableName, f.columnName);
				//append(e).append(".").append(c).append(" as ").append(e).append("_").append(c)
				String colAlias = pe + "_" + f.columnName;
				columnProjectionMap.put(qualifiedCol, qualifiedCol + " as " + colAlias);
			} else {
				String e = pe+f.index;
				boolean isRequired = required & f.required;
				...
				prepareSqlQuery(edJoin, sbColumns, sbJoins, edJoin.tableName, e, isRequired);
			}
		}
	}
