...
public abstract class EntityManager {
	...
	public static EntityDefinition getEntityDefinitionOrThrow(Class<?> clazz) {
	...
	public ContentValues getContentValues(Object entity){
		if (entity == null) {
			throw new IllegalArgumentException("Entity is null");
		}
	...
	public <T> T get(Class<T> clazz, Object id) {
		...
		try {
			if (c.moveToFirst()) {
				try {
					return (T)loadFromCursor("e", c, ed, true);
				} catch (Exception e) {
	...
	public <T> List<T> list(Class<T> clazz) {
		...
		try {
			...
			while (c.moveToNext()) {
				try {
					T t = (T)loadFromCursor("e", c, ed, true);
					list.add(t);
				} catch (Exception e) {
	...
	public static <T> T loadFromCursor(Cursor c, Class<T> clazz) {
		return loadFromCursor("e", c, clazz, true);
	}

	public static <T> T loadFromCursor(String pe, Cursor c, Class<T> clazz, Boolean loadEntityProps) {
		EntityDefinition ed = getEntityDefinitionOrThrow(clazz);
		try {
			return (T)loadFromCursor(pe, c, ed, loadEntityProps);
		} catch (Exception e) {
			throw new PersistenceException("Unable to load entity of type "+clazz+" from cursor", e);
		}
	}

	private static <T> T loadFromCursor(String pe, Cursor c, EntityDefinition ed, Boolean loadEntityProps) throws Exception {
		String colPrefix = "";
		if (Utils.isNotEmpty(pe)){
			colPrefix = pe + "_";
		}

		int idIndex = c.getColumnIndexOrThrow(colPrefix + "_id");
		if (c.isNull(idIndex)) {
			return null;
		}
		...
		for (int i = 0; i<count; i++) {
			...
			Object value = null;
			if (fi.type.isPrimitive()) {
				value = fi.type.getValueFromCursor(c, colPrefix + fi.columnName);
			} else if (loadEntityProps) {
				EntityDefinition eed = getEntityDefinitionOrThrow(fi.field.getType());
				value = loadFromCursor(pe+fi.index, c, eed, loadEntityProps);
			}
